...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* lineTok[3];
            lineTok[0] = strtok(line, " ");
            for(int i = 1;i < 3;i++)
            {
                lineTok[i] = strtok(NULL, " ");
            }
            if(strcmp(lineTok[0],"GET") != 0)
            {
                error(405);
                continue;
            }
            else if(lineTok[1][0] != '/' || (strchr(lineTok[1],'.') == NULL))
            {
                error(501);
                continue;
            }
            else if(strchr(lineTok[1],'"') != NULL)
            {
                error(400);
                continue;
            }
            else if(strcmp(lineTok[2],"HTTP/1.1") == 0)
            {
                error(505);
                continue;
            }
            ...
            char* queryId = strchrnul(lineTok[1],'?');
            char query[strlen(queryId)];
            strcpy(query,(strlen(queryId) == 0) ? "\0" : queryId + 1);
            

            // TODO: concatenate root and absolute-path
            int absPathId = strlen(lineTok[1]) - strlen(queryId);
            char absPath[absPathId + 1];
            strncpy(absPath,lineTok[1],absPathId);
            absPath[absPathId] = '\0';
            char path[strlen(absPath) + strlen(root)+1];
            strcpy(path, root);
            strcat(path,absPath);
            ...
            char* exId = strchrnul(absPath,'.');
            char extension[strlen(exId)];
            strcpy(extension,exId + 1);
